...
function getDays(date, finalDay) {
  const days = [];
  for(let i = 1; i <= finalDay; i++) {
    if(i === new Date().getDate() && date.getMonth() === new Date().getMonth() && date.getFullYear() === new Date().getFullYear()) {
      days.push(<div className={styles.today}>{i}</div>);
    } else {
      days.push(<div>{i}</div>);
    }
  }
...
function getPrevDays(date, prevLastDay) {
  date.setDate(1);
  const firstDayIndex = date.getDay();
  const days = [];

  for(let i = firstDayIndex; i > 0; i--) {
    days.push(<div className={styles.prevDate}>{prevLastDay - i + 1}</div>)
  }
...
function getNextDays(date) {
  ...
  for(let i = 1; i <= nextDays; i++) {
    days.push(<div className={styles.nextDate}>{i}</div>)
  }
...
export default function Resources() {
  ...
  const [prevDays, setPrevDays] = useState(getPrevDays(date, prevLastDay));
  const [currentDays, setCurrentDays] = useState(getDays(date, lastDay));
  const [nextDays, setNextDays] = useState(getNextDays(date));
  const [combinedDays, setCombinedDays] = useState(prevDays.concat(currentDays.concat(nextDays)));
  ...
  useEffect(() => {
    ...
    setPrevDays(getPrevDays(date, prevLastDay));
    setCurrentDays(getDays(date, lastDay));
    setNextDays(getNextDays(date));
    setCombinedDays(prevDays.concat(currentDays.concat(nextDays)));
  }, [date, lastDay, prevLastDay, prevDays, currentDays, nextDays])
    ...
    >
        <div className={styles.leftCol}>
          ...
          <div id="destinations" className={styles.destinations}>
            <label for="destinations" className={styles.label}>Destination</label>
          ...
          <div className={styles.travelers}>
            <label for="destinations" className={styles.label}>Travelers</label>
